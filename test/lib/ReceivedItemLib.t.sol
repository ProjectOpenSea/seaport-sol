...
contract ReceivedItemLibTest is BaseTest {
    ...
    function testRetrieveDefault(
        ...
        ReceivedItem memory receivedItem =
            ReceivedItem(ItemType(itemType), token, identifier, amount == 0 ? 1 : amount, recipient);
        ReceivedItemLib.saveDefault(receivedItem, "default");
        ReceivedItem memory defaultReceivedItem = ReceivedItemLib.fromDefault("default");
    ...
    function testComposeEmpty(
        ...
        itemType = uint8(bound(itemType, 0, 5));
        ReceivedItem memory receivedItem = ReceivedItemLib.empty().withItemType(ItemType(itemType)).withToken(token)
            .withIdentifier(identifier).withAmount(amount).withRecipient(recipient);
        assertEq(
            receivedItem,
            ReceivedItem({
    ...
    function testCopy() public {
        ReceivedItem memory receivedItem = ReceivedItem(ItemType(1), address(1), 1, 1, payable(address(1234)));
    ...
    function testRetrieveDefaultMany(
        ...
        for (uint256 i = 0; i < 3; i++) {
            ...
            receivedItems[i] = ReceivedItem(ItemType(itemType[i]), token[i], identifier[i], amount[i], recipient[i]);
        }
        ReceivedItemLib.saveDefaultMany(receivedItems, "default");
        ReceivedItem[] memory defaultReceivedItems = ReceivedItemLib.fromDefaultMany("default");
        for (uint256 i = 0; i < 3; i++) {
