...
contract AdditionalRecipientLibTest is BaseTest {
    ...
    function testRetrieveDefault(uint256 amount, address payable recipient) public {
        ...
        AdditionalRecipient memory defaultAdditionalRecipient = AdditionalRecipientLib.fromDefault("default");
    ...
    function testComposeEmpty(uint256 amount, address payable recipient) public {
        AdditionalRecipient memory additionalRecipient =
            AdditionalRecipientLib.empty().withAmount(amount).withRecipient(recipient);
        assertEq(additionalRecipient, AdditionalRecipient({amount: amount, recipient: recipient}));
    }

    function testCopy() public {
        AdditionalRecipient memory additionalRecipient =
            AdditionalRecipient({amount: 1, recipient: payable(address(1))});
    ...
    function testRetrieveDefaultMany(uint256[3] memory amount, address payable[3] memory recipient) public {
        AdditionalRecipient[] memory additionalRecipients = new AdditionalRecipient[](3);
        for (uint256 i = 0; i < 3; i++) {
            additionalRecipients[i] = AdditionalRecipient({amount: amount[i], recipient: recipient[i]});
        }
        AdditionalRecipientLib.saveDefaultMany(additionalRecipients, "default");
        AdditionalRecipient[] memory defaultAdditionalRecipients = AdditionalRecipientLib.fromDefaultMany("default");
        for (uint256 i = 0; i < 3; i++) {
