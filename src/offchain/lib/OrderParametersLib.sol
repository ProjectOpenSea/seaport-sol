...
library OrderParametersLib {
     ...
     * @param defaultName the name of the default for retrieval
     */
    function saveDefault(OrderParameters memory orderParameters, string memory defaultName) internal {
        ...
        returns (OrderComponents memory components)
    {
        ...
        ConsiderationItem[] memory consideration = new ConsiderationItem[](parameters.totalOriginalConsiderationItems);
        for (uint256 i = 0; i < parameters.totalOriginalConsiderationItems; i++) {
            consideration[i] = parameters.consideration[i].copy();
        }
